...
function findImpuesto(totalImpuestoArr: TotalImpuesto[], codigo: string, codigoPorcentaje?: string): TotalImpuesto {
	const result = totalImpuestoArr.find(totalImpuesto => {
	...
	if (!result) {
		return {valor: [], baseImponible: [], codigo: [], codigoPorcentaje: [], tarifa: []};
	}

	return result;
}

export default function rowifyNotaCredito(autorizacion: Autorizacion): RowNotaCredito {
	...
	const fecha = processDate(gS(notaCredito, 'infoNotaCredito.0.fechaEmision.0'));
	const numAutorizacion = gS(autorizacion, 'numeroAutorizacion.0');
	...
	const totalImpuestoArr = gO(notaCredito, 'infoNotaCredito.0.totalConImpuestos.0.totalImpuesto') as TotalImpuesto[];
	const impuestoIva = findImpuesto(totalImpuestoArr, '2', '2');
	const impuestoIce = findImpuesto(totalImpuestoArr, '3');
	const impuesto0 = findImpuesto(totalImpuestoArr, '2', '0');

	const {valor: [iva = ''], baseImponible: [tarifa12 = '']} = impuestoIva;
	const {valor: [ice = ''], baseImponible: [tarifa15 = '']} = impuestoIce;
	const {baseImponible: [tarifa0 = '']} = impuesto0;
